...
function isCurrentMonthActive(creature, hemisphere) {
...
function isMonthActive(month, creature, hemisphere) {
  ...
  return monthRanges.some((rng) => {
    return (rng.start <= month && rng.end >= month) ||
      (rng.start <= month && rng.start > rng.end) ||
      (rng.start >= month && rng.start > rng.end && rng.end >= month)
  });
}
